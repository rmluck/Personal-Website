    ...
    return (
        ...
        >
            ...
            >
                ...
                <motion.div
                    className={`relative flex items-center justify-center h-full max-w-7xl transition-all duration-300`}
                    animate={{
                ...
                >
                                    ...
                                    {loaded && (
                                        <>
                                            <div className="absolute top-3 left-3 z-50 flex items-center gap-2">
                                                <button
                                                    onClick={() => closeModal()}
                                                    className="rounded-full p-2 border-2 border-transparent  backdrop-blur-xl bg-pro200/50 dark:bg-pro900/50 text-pro900/75 dark:text-pro200/75 hover:bg-accent/30 hover:text-accent hover:border-accent transition focus:outline-none cursor-hover cursor-none clickable"
                                                >
                                                    {navigation ? (
                                                        <X className="h-5 w-5" />
                                                    ) : (
                                            ...
                                            <div className="absolute top-3 right-3 z-50 flex items-center gap-2">
                                                <button
                                                    ...
                                                    className={`rounded-full p-2 border-2 border-transparent backdrop-blur-xl transition focus:outline-none cursor-hover cursor-none clickable ${showInfo ? "bg-accent/30 text-accent border-accent" : "bg-pro200/50 dark:bg-pro900/50 text-pro900/75 dark:text-pro200/75 hover:bg-accent/30 hover:text-accent hover:border-accent"}`}
                                                    title="Image context"
                                                >
                    ...
                    {loaded && (
                        <>
                            {navigation && (
                                <>
                                    ...
                                    {index + 1 < images.length && (
                                        <button
                                            className={`absolute top-1/2 -translate-y-1/2 z-50 rounded-full p-3 border-2 border-transparent backdrop-blur-xl bg-pro200/50 dark:bg-pro900/50 text-pro900/75 dark:text-pro200/75 hover:bg-accent/30 hover:text-accent hover:border-accent transition focus:outline-none cursor-hover cursor-none clickable right-3`}
                                            onClick={() => changePhotoId(index + 1)}
                                        >
                            ...
                            {/* Top Right Controls */}
                            {/* <div className={`absolute ${isPortrait ? "-top-10" : "top-5"} right-3 z-50 flex items-center gap-2`}>
                                <a
                                    href={currentImage.urls.original}
                                    className="rounded-full p-2 border-2 border-transparent backdrop-blur-xl bg-pro200/50 dark:bg-pro900/50 text-pro900/75 dark:text-pro200/75 hover:bg-accent/30 hover:text-accent hover:border-accent transition focus:outline-none cursor-hover cursor-none clickable"
                                    target="_blank"
                            ...
                            {/* Top Left Controls */}
                            {/* <div className={`absolute ${isPortrait ? "-top-10" : "top-5"} left-3 z-50 flex items-center gap-2`}>
                                <button
                                    onClick={() => closeModal()}
                                    className="rounded-full p-2 border-2 border-transparent backdrop-blur-xl bg-pro200/50 dark:bg-pro900/50 text-pro900/75 dark:text-pro200/75 hover:bg-accent/30 hover:text-accent hover:border-accent transition focus:outline-none cursor-hover cursor-none clickable"
                                >
                ...
                <AnimatePresence mode="wait">
                    {showInfo && loaded && (
                        <motion.div
                            ...
                            // style={{ transform: `translateX(${showInfo ? '0%' : '100%'})` }}
                        >
                            <div className="p-6 h-full overflow-y-auto">
                                <div className="flex items-center justify-between mb-6">
                                    <h3 className="text-lg font-semibold text-pro900 dark:text-pro100 font-regular">
                                        Image Context
                                    </h3>
                                    <button
                                        onClick={handleClosePanel}
                                        className="rounded-full p-1 hover:bg-pro200 dark:hover:bg-pro800 transition cursor-hover cursor-none clickable text-pro600 dark:text-pro400"
                                    >
                                        <X className="h-5 w-5" />
                                    </button>
                                </div>
                ...
                {/* Bottom Carousel */}
                {navigation && (
                    <div className="fixed inset-x-0 bottom-0 z-40 overflow-hidden bg-gradient-to-b from-pro200/0 to-pro200/60 dark:from-pro900/0 dark:to-pro900/60">
                        <motion.div
                            initial={false}
